...
PROMPT = """You are a senior data assistant. PLAN BY WRITING PYTHON CODE USING TINYDB.
        ...
        - Derive ALL filters/parameters from user_request (shape/keywords, price ranges \
            "under/over/between", stock mentions,
          quantities, buy/return intent). Do NOT hard-code values.
        - Build TinyDB queries dynamically with Query(). If a constraint isn't in user_request, \
            don't apply it.
        ...
        - If any requested item lacks sufficient stock, do NOT mutate anything; reply with \
            STATUS="insufficient_stock".

        HUMAN RESPONSE REQUIREMENT (hard):
        - You MUST set a variable named `answer_text` (type str) with a short, customer-friendly \
            sentence (1-2 lines).
        - This sentence is the only user-facing message. No dataframes/JSON, no boilerplate \
            disclaimers.
        - If nothing matches, politely say so and offer a nearby alternative (closest style/price) \
            or a next step.

        ACTION POLICY:
        - If the request clearly asks to change state (buy/purchase/return/restock/adjust):
            ACTION="mutate"; SHOULD_MUTATE=True; perform the change and write a matching \
                transaction row.
          Otherwise:
            ACTION="read"; SHOULD_MUTATE=False; simulate and explain briefly as a dry run \
                (in logs only).
        ...
        - no_match: No items satisfy the filters → suggest the closest in style/price, or invite a \
            different range.
        - insufficient_stock: Item found but stock < requested qty → state available qty and offer \
            the max you can fulfill.
        - invalid_request: Unable to parse essential info (e.g., quantity for a purchase/return) → \
            ask for the missing piece succinctly.
        - unsupported_intent: The action is outside the store's capabilities → provide the nearest \
            supported alternative.
        - In all cases, keep the tone helpful and concise (1-2 sentences). Put technical details \
            (e.g., ACTION/DRY RUN) only in stdout logs.
        ...
        3) If mutate: validate stock, update inventory, insert a transaction (new id, amount, \
            balance, timestamp).
        4) ALWAYS set:
        ...
        - no_match: "We don't have round frames under $100 in stock right now, but our Moon round \
                    frame is available at $120."
...
## ------------------------------------------------------ ##
def generate_llm_code(prompt: str, *, inventory_tbl, transactions_tbl, model: str = "gpt-4.1-mini",
...
utils.print_html(json.dumps(round_sunglasses, indent = 2), title = "Inventory Status: Round " \
                                                                    "Sunglasses")
## ------------------------------------------------------ ##
prompt_round = "Do you have any round sunglasses in stock that are under $100?"

full_content_round = generate_llm_code(prompt_round, inventory_tbl = inventory_tbl,
...
def execute_generated_code(code_or_content: str, *, db, inventory_tbl, transactions_tbl,
...
utils.print_html(json.dumps(aviators, indent = 2), title = "Inventory status: Aviator sunglasses " \
                                                            "before return")
...
## ------------------------------------------------------ ##
def customer_service_agent(question: str, *, db, inventory_tbl, transactions_tbl,
                           model: str = "o4-mini", temperature: float = 1.0, reseed: bool = False,
                           ) -> dict:
    if reseed:
